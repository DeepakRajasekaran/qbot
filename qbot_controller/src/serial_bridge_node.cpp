...
{
  if (std::abs(msg->linear.x) < 1e-3 && std::abs(msg->angular.z) < 1e-3) {
    sendCommand("s");
    if (m_modeResetTimer) {
      m_modeResetTimer->reset();
    } else {
      m_modeResetTimer = this->create_wall_timer(
        500ms, std::bind(&SerialBridgeNode::resetModeCallback, this));
    }
  } else {
    std::stringstream ss;
...
void SerialBridgeNode::resetModeCallback()
{
  sendCommand("m1");
  if (m_modeResetTimer) {
    m_modeResetTimer->cancel();
    m_modeResetTimer.reset();
  }
}
